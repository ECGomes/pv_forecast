...
class MetricsEE(MetricsBase):
    ...
    def cmd_nrmse(self, state_gt, state_pred, use_mean=False):
        """
        Calculates the normalized RMSE
        use_mean: uses state_gt mean to normalize RMSE
        use_mean set to False: use the state_gt.max - state_gt.min to normalize
        """

        temp_rmse = self.cmd_rmse(state_gt, state_pred)
        if use_mean:
            return temp_rmse / state_gt.mean()
        else:
            return temp_rmse / (state_gt.max() - state_gt.min())

    
    def cmd_mape(self, state_gt, state_pred):
        """
        ...
        temp_mape = sklearn.metrics.mean_absolute_percentage_error(state_gt, state_pred)
